          ...
          <div id="language<?php echo $language['language_id']; ?>">
            <table class="form">
              <tr>
                ...
                <td><input type="text" onblur="locdau();" id="news_description[<?php echo $language['language_id']; ?>][name]" name="news_description[<?php echo $language['language_id']; ?>][name]" size="100" value="<?php echo isset($news_description[$language['language_id']]) ? $news_description[$language['language_id']]['name'] : ''; ?>" />
          ...
          <table class="form">
			...
			<tr>
              ...
              <td><div class="scrollbox">
                  ...
                  <?php $class = ($class == 'even' ? 'odd' : 'even'); ?>
                  <div class="<?php echo $class; ?>">
                    <?php if (in_array($download['download_id'], $news_download)) { ?>
                    <input type="checkbox" name="news_download[]" value="<?php echo $download['download_id']; ?>" checked="checked" />
                    <?php echo $download['name']; ?>
                    <?php } else { ?>
                    <input type="checkbox" name="news_download[]" value="<?php echo $download['download_id']; ?>" />
                    <?php echo $download['name']; ?>
                    <?php } ?>
                  </div>
                  <?php } ?>
            ...
            <tr>
              <td>&nbsp;</td>
              <td><div id="news-related" class="scrollbox">
                  <?php $class = 'odd'; ?>
...
<script type='text/javascript'>
	...
	var str = (document.getElementById("news_description[<?php echo $this->config->get('config_language_id'); ?>][name]").value);// lấy chuỗi dữ liệu nhập vào từ form có tên title
	...
	str = '<?php echo $now;?>-' + str + '.html'; //thêm .html vào sau chuỗi
...
$.widget('custom.catcomplete', $.ui.autocomplete, {
	_renderMenu: function(ul, items) {
		var self = this, currentcat = '';
		
		$.each(items, function(index, item) {
			if (item.cat != currentcat) {
				ul.append('<li class="ui-autocomplete-cat">' + item.cat + '</li>');
...
function attributeautocomplete(attribute_row) {
	$('input[name=\'news_attribute[' + attribute_row + '][name]\']').catcomplete({
		...
		source: function(request, response) {
			$.ajax({
				url: 'index.php?route=content/attribute/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
				dataType: 'json',
				success: function(json) {	
					response($.map(json, function(item) {
						return {
							cat: item.attribute_group,
							label: item.name,
...
$('input[name=\'option\']').catcomplete({
	...
	source: function(request, response) {
		$.ajax({
			url: 'index.php?route=content/option/autocomplete&token=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request.term),
			dataType: 'json',
			success: function(json) {
				response($.map(json, function(item) {
					return {
